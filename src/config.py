...
# 粤语翻译默认提示词模板存放位置（纯文本，可直接手改；含 {text} 占位，
# 运行时被输入源文案替换；不含 {text} 时源文案直接拼在模板末尾）。
# 采用 Hy-MT2 官方"参考翻译"指令格式 + 防照抄约束：示例只示范粤语用字与
# 句式（否则 1.8B 会在输入与示例相近时直接照搬示例答案），并要求逐句
# 翻译、禁止回显原文、统一香港繁体。文件内容即 web「粤语翻译」页右侧
# 提示词框的初始值（可在页面内临时修改，持久化改此文件）。示例文件
# 缺失时 hymt2 用内置简化模板兜底。
HYMT2_PROMPT_FILE = _env(
    "HYMT2_PROMPT_FILE", os.path.join(_PROJECT_ROOT, "hymt2_prompt.txt"))
